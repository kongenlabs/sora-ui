...
function RadioGroup({ className, color = 'primary', ...props }: RadioGroupProps) {
  return (
    <RadixRadioGroup
      className={cx('group grid gap-3', className)}
      data-color={color}
...
function RadioGroupIndicator({ className, children, ...props }: RadioGroupIndicatorProps) {
  return (
    <RadixRadioGroupIndicator
      className={cx('grid place-content-center', className)}
      data-slot='radio-group-indicator'
      {...props}
    >
      {children ?? <CircleIcon className='size-4 fill-current' />}
    </RadixRadioGroupIndicator>
  );
}
